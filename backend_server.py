...
class CitationAPI:
    ...
    def process_message(self, message: str, history: list[dict[str, str]] = None) -> dict[str, Any]:
        ...
        if citations and self.fact_checker:
            try:
                ...
                self.pending_tasks[task_id] = {"response": response, "citations": citations}
                ...
                def fact_check_with_progress(citations_list):
                    ...
                    return self.fact_checker.fact_check_citations(
                        citation_objects,
                        progress_callback=lambda progress, result: async_processor.update_progress(
                ...
                async_processor.create_task(
                    task_id, fact_check_with_progress, citations, timeout=45.0
                )
        ...
        return {"response": response, "citations": citations, "task_id": task_id}
    ...
    def _on_fact_check_complete(
        self, task_id: str, fact_check_results: list = None, error: str = None
    ):
    ...
    def get_task_status(self, task_id: str) -> dict:
        ...
        if task.status == TaskStatus.COMPLETED and "fact_check_results" in task_data:
            response["result"] = {"fact_check_results": task_data["fact_check_results"]}
    ...
    def get_system_status(self) -> dict:
        ...
        chat_model_ok = bool(
            self.chat_model
            and hasattr(self.chat_model, "validate_setup")
            and self.chat_model.validate_setup()
        )
        search_client_ok = bool(
            self.search_client
            and hasattr(self.search_client, "validate_setup")
            and self.search_client.validate_setup()
        )
        ner_extractor_ok = bool(
            self.ner_extractor
            and hasattr(self.ner_extractor, "validate_setup")
            and self.ner_extractor.validate_setup()
        )
        fact_checker_ok = bool(
            self.fact_checker
            and hasattr(self.fact_checker, "validate_setup")
            and self.fact_checker.validate_setup()
        )
    ...
    def get_usage_stats(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                "success_rate": daily_stats.successful_calls
                / max(1, daily_stats.total_calls)
                * 100,
